<%@page import="com.chetan.model.VaccineLogs"%>
...
<body>
	...
	<div class="main-container">
			...
			<div class="pd-20 card-box mb-30" style="margin-left: 6rem;background: #fff;">
					<h3>View Upcoming All Child Vaccines(Next 30 Days)</h3>
					<br>
	...
	List<VaccineLogs> vaccineLogsList = (List<VaccineLogs>)request.getAttribute("vaccineLogsList");
	
	for(Vaccine vac : vaccineList)
	{
		for(Child dob : childList)
		{
			LocalDate today=LocalDate.now();
			DateTimeFormatter formatter=DateTimeFormatter.ofPattern("yyyy-MM-dd");
			LocalDate dateOfBirth=LocalDate.parse(dob.getDob(),formatter);
			Period per=Period.between(dateOfBirth,today);
			int month=per.getYears()*12+per.getMonths();
			int nextMonth=month+1;
			if(nextMonth == vac.getMonths())
			{
				for(VaccineLogs vaccLog : vaccineLogsList)
				{
					if(dob.getcId()!= vaccLog.getChildId())
					{
						int i=1;
	%>
		...
		<tr>
			<td><%=i%></td>
			<td><%=dob.getcId() %></td>
			<td><%=dob.getChildName()%></td>
			<td><%=dob.getParentName()%></td>
			<td><%=dob.getDob()%></td>
			<td><%=dob.getParentContact()%></td>
			<td><%=dob.getAddress()%></td>
			<td><%=vac.getMonths()%></td>
			<td><%=vac.getVaccineName()%></td>
		</tr>
