...
<body>
	...
	<%
		VaccineLogs vacLog = (VaccineLogs) request.getAttribute("vaccineLog");
		if (vacLog != null) {
		%>
			...
			<div class="pd-20 card-box mb-30" style="margin-left: 6rem;background: #fff;">
					...
					<div class="table-responsive">
						<table class="table" border="2" style="text-align: center;font-size: 5rem;">
							...
							<tbody>
							<tr>
            <td><%=vacLog.getVlId()%></td>
            <td><%= vacLog.getChildId() %></td>
            <td><%= vacLog.getVaccineName() %></td>
            <td><%= vacLog.getDate() %></td>
            <td><%= vacLog.getAgeMonths() %></td>
        </tr>
    </table>
<%
} else {
%>
    <p>No vaccine logs available.</p>
<%
}
%>
